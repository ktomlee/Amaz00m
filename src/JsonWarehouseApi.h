...
class JsonMusicLibraryApi : public MusicLibraryApi {
 private:
  ...
  bool readString(std::string& str, size_t size) {
    ...
    // receive message of size param size

    while(size > 0)
    {
      size_t bytes_to_read = std::min(size, (size_t)256);
      
      if (!socket_.read_all(cbuff, bytes_to_read)) {
        return false;
      }
      
      for(int i=0; i<bytes_to_read; i++)
      {
        str.append(cbuff+i, 1);
      }
      size -= bytes_to_read;
    }
  ...
  bool recvJSON(JSON& jout) {
    ...
    std::string str;
    if (!readString(std::ref(str), size)) {
      return false;
    }
